...
**Nota de verificación sobre el paso 4 (Principio D) — la premisa era PARCIAL.** La verificación adversarial sobre el motor instalado (`$DSH_NM` = `/Applications/DSH Desktop.app/Contents/Resources/app/node_modules/@deepseek-ai`, copia byte-idéntica a `desktop/src/dsh-desktop/node_modules/@deepseek-ai`) confirma el núcleo de la premisa pero corrige su alcance. **El paso 4 sigue siendo necesario; no se reordena ni se elimina.**
