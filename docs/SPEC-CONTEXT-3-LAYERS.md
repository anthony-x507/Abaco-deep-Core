...
**Convención de rutas:** `$DSH_NM` = `/Applications/ABACO DEEP HARNES.app/Contents/Resources/app/node_modules/@deepseek-ai` — la copia del motor que **ABACO ejecuta**, y por tanto la **AUTORITATIVA** para toda cita `archivo:línea` de esta spec. **No** es la de la app **DeepSeek Desktop** (`/Applications/DSH Desktop.app/.../@deepseek-ai`), que es **otro producto** y no tiene autoridad sobre ABACO. Tercera copia, entrada de build del repo: `desktop/src/dsh-desktop/node_modules/@deepseek-ai`. Hoy las tres son byte-idénticas; la cita nombra **siempre** la autoritativa. Trampa y procedimiento si divergen: `docs/HANDOFF-FASE5.md` §7.12.
...
**El preset `abaco` nunca se activó** *(al medir: ése era el estado entonces; hoy está instalado y adoptado — ver la retractación de abajo)*. El repo lo define con 0.6/0.08/16384:
...
**Corrección a la auditoría previa — y RETRACTACIÓN de esa corrección (2026-09-10):**

> **Primero, de qué perfil se lee.** Hay **dos** perfiles bajo `~/Library/Application Support/`, con nombres casi iguales y **ambos con `harness/`**: `abaco-deep-core/` (**el nuestro**, ABACO DEEP HARNES) y `dsh-desktop/` (la app **DeepSeek Desktop**, que sirve otra conversación — **INTOCABLE**). **Regla: sin nombrar el perfil, el dato no se usa** (R13). La corrección que este §2.1 traía leyó el perfil **vecino** y revirtió un hallazgo que era **cierto**.

- **`$DSH_HOME` del perfil de ABACO** = `~/Library/Application Support/abaco-deep-core/harness`. **No es** `~/.dsh` (comprobado: **no existe**) y **no es** `…/dsh-desktop/harness`, que es el perfil de la app DeepSeek Desktop. *(**RETRACTADO.** Decía: «`$DSH_HOME` real = `/Users/a507/Library/Application Support/dsh-desktop/harness` (no `~/.dsh`)». La mitad «no `~/.dsh`» era cierta; el perfil, no.)*
- **`settings.yaml` del perfil de ABACO SÍ tiene hoy la clave `agent-presets`**: `…/abaco-deep-core/harness/settings.yaml:1-2` → `default: abaco`. *(**RETRACTADO.** Decía: «**`settings.yaml` SÍ tiene la clave `agent-presets`**: `$DSH_HOME/settings.yaml:11-12` → `default: cordis`. La auditoría afirmaba que faltaba.» Ese `:11-12` con `default: cordis` **existe**, pero en `…/dsh-desktop/harness/settings.yaml` — el perfil **vecino**, no el de ABACO. La auditoría a la que esta línea "corregía" **tenía razón**, y su conclusión de fondo —«el preset activo no es `abaco`»— también ha dejado de valer.)*
- **Antes ≠ después: el hallazgo original era CIERTO.** En el perfil de ABACO la clave `agent-presets` **FALTABA**, y por **ausencia** de clave el roster caía al default de la **composición**, `standard` (`$DSH_NM/dsh-web-app/cordis.patch.yml:440-444`) — **no** a `cordis` por valor explícito. Ésa es la causa raíz del Bug 2. **Hoy la clave está y dice `abaco` porque el arreglo del Bug 2 la escribió** (commit `7936852`), no porque hubiera estado siempre. La prueba está en disco, en el marcador que la propia app escribe: `…/abaco-deep-core/harness/.agent-presets/.abaco-context-default.json` → `{"applied": true, "presetId": "abaco", "replaced": "standard", "at": "2026-09-10T21:34:14.122Z"}`. Ese `"replaced": "standard"` es el default efectivo **anterior**, tal como lo registró el plugin al sustituirlo.
- **Estado de instalación, movido** (medido el 2026-09-10, 20:12 EDT). En el perfil de ABACO **hoy SÍ existen** `$DSH_HOME/.agent-presets/` (con `abaco/agent.cordis.yml`, `abaco/preset.yml`, `.abaco-context.json` y `.abaco-context-default.json`) y `$DSH_HOME/abaco-memory/vault/`. *(**RETRACTADO.** Decía: «El marcador de instalación **no existe**: no hay `$DSH_HOME/.abaco-context-default.json` ni el directorio `$DSH_HOME/.agent-presets/` (ausente por completo). … y tampoco existe.» Esa frase **no nombraba el perfil** —la violación de R13 que produjo todo el error—; y sea cual fuera el perfil medido, **hoy es falso para el de ABACO**, donde el arreglo del Bug 2 instaló preset y marcadores: `installedAt: 2026-09-10T21:54:36.291Z`. El marcador que el preset documenta sigue siendo `.agent-presets/.abaco-context.json` (`.../agent.cordis.yml:26-29`).)*

**Conclusión Capa 1** *(estado al medir — ver la retractación de arriba)*: corría 0.8/0.16/8192 del stock, no la política ABACO. **Hoy el preset ABACO está instalado y seleccionado** —`default: abaco` en el `settings.yaml` del perfil de ABACO y el marcador en `"applied": true, "replaced": "standard"`—, con su fila cableada a 0.90/0.12/8192; que el motor **ejecute** esa política en vivo sigue siendo **[NO VERIFICADO]** (requiere la telemetría de fase 0, §8.5), y **0.6 tampoco es el 90% que pide el dueño** (ver §3.A).
...
**Está VACÍA.** En el perfil de ABACO (`$DSH_HOME` = `…/abaco-deep-core/harness`, R13) **hoy el directorio `abaco-memory/` SÍ existe** — la app lo creó al arrancar (`Sep 10 17:34`) y contiene **un único `vault/` vacío**: cero ficheros, cero recuerdos. *(**RETRACTADO.** Decía: «`$DSH_HOME/abaco-memory/` **no existe** (la auditoría decía "0 ficheros"; lo verificado hoy es más fuerte: el directorio no existe)». No nombraba el perfil —R13— y hoy es falso para el de ABACO. La conclusión de fondo **no cambia**: nunca se escribió un recuerdo.)* Existe además `packages/abaco-vault/` con almacén propio (`desktop/src/dsh-desktop/build/dsh-desktop.patch.yml:125-126`).
...
**Qué crear/modificar.** (1) Valor `thresholdRatio: 0.9` donde el preset activo lo lea. (2) La activación real del preset: crear `$DSH_HOME/.agent-presets/` + marcador, o cambiar `…/abaco-deep-core/harness/settings.yaml:1-2` a `default: abaco` — **decisión de producto**, porque `default` afecta a toda sesión nueva. **Estado 2026-09-10: ambas cosas ya ocurrieron** — el arreglo del Bug 2 (`7936852`) instaló el preset y escribió `agent-presets: default: abaco` en el `settings.yaml` del perfil de ABACO. Ver §2.1 y R13.
...
**Estado.** No implementado. El pruner **nunca lee `isError`** (`$DSH_NM/dsh-compaction-tool-result-pruner/lib/index.js:137` — `pruneSession`; **0 ocurrencias** de `isError` en todo el archivo), aunque el campo existe (`$DSH_NM/dsh-llm/lib/types/message.js:78`, y se propaga al mensaje en `$DSH_NM/dsh-llm/lib/index.js:84`). Hoy un resultado de error se trunca **igual** que uno normal.

> **Nota de verificación (motor instalado, revisión adversarial).** La premisa que justificaba este paso —«el pruner nunca lee `isError` … los errores se truncan y el LLM los resume como una bala más»— es **PARCIAL**. El paso 4 **sigue siendo necesario**, pero cambia su alcance y su justificación:
>
> - **Ya cubierto por el motor (no hay que implementarlo):** el prune **no destruye** la marca. La sustitución conserva el bloque entero (`...result` en `dsh-compaction-tool-result-pruner/lib/index.js:157-160`) y reanexa `event.data` completo (`:170-172`), así que `isError` **sobrevive** al truncado aunque nada lo consulte. Y la directiva de compactación tiene una sección **dedicada**, `"## Errors and Fixes"` (`$DSH_NM/dsh-compaction-basic/lib/index.js:234`): los errores no caen «en una bala más» genérica.
> - **Sigue faltando (esto es el paso 4):** (a) no existe **ninguna guarda que actúe** sobre `isError` — `pruneContent` (`:91-124`) se aplica **sin condición** a todo `tool/result` que pase de `thresholdChars`, error incluido; y (b) `buildSummarizationInput` (`$DSH_NM/dsh-compaction-basic/lib/index.js:651-653`) **no filtra nada**, de modo que el error entra íntegro en el corpus a resumir, y la sección «Errors and Fixes» pide **condensar** («how it was resolved»), no transcribir literal.
> - **Corrección al «Punto de enganche» de abajo:** el camino de overflow **sí** pasa por la misma función — `compactIfNeeded` llama a `prune.pruneSession` también con `trigger === "context-overflow"` (`$DSH_NM/dsh-compaction-basic/lib/index.js:872`) —, así que una guarda dentro de `pruneSession` cubre **ambas** rutas (presión: `:886`; overflow: `:872`). El hueco propio del overflow es otro: después llama a `selectCompactableRange(..., 0)` (`:875`) y resume **todo** el historial —error incluido— con guarda o sin ella en el pruner.

**Punto de enganche (dos sitios, ambos necesarios).**
1. **Truncado mecánico:** `pruneSession`, `$DSH_NM/dsh-compaction-tool-result-pruner/lib/index.js:137-194`. Se invoca desde el motor vía `ctx.get("toolResultPruner")` (`$DSH_NM/dsh-compaction-basic/lib/index.js:869`) en ambas rutas: presión `:886`, overflow `:872`. Añadir guarda: si `event.data.message.content[0].isError === true`, no truncar.
2. **Reescritura por LLM:** el prompt de compactación (`:220-255`), cuya sección "Errors and Fixes" (`:234`) hoy invita a condensar. Debe exigir transcripción literal del mensaje de error.

**Qué crear/modificar.** Guarda `isError` en el pruner (parche del fork o subclase propia en fase 3) + regla literal en la plantilla. **Ojo:** una guarda dentro de `pruneSession` cubre las dos rutas de prune (presión y overflow), pero **no** impide que el overflow resuma el error: para eso hace falta lo del punto 2.
...
| R9 | **Duplicar la política en dos sitios** (`…/abaco-deep-core/harness/settings.yaml:1-2` y el preset `abaco`) | Cambiar el umbral en un sitio y no en el otro produce un comportamiento que no coincide con la configuración visible. Síntoma: el ratio medido no corresponde al valor editado. |
...
| R13 | **La trampa de los dos perfiles** — `abaco-deep-core/` (ABACO) frente a `dsh-desktop/` (DeepSeek Desktop) | Los dos perfiles viven bajo `~/Library/Application Support/`, **ambos contienen `harness/`**, y sus nombres se parecen. **Ya ha producido tres auditorías distintas con datos falsos**, y esta spec fue una de ellas: el §2.1 leyó `…/dsh-desktop/harness/settings.yaml:11-12` (`default: cordis`) y se lo atribuyó al perfil de ABACO, revirtiendo así un hallazgo cierto (la causa raíz del Bug 2). Síntoma: la spec afirma un `$DSH_HOME`, un `settings.yaml` o un marcador que **no son los del producto**. **Contramedida — citar siempre el perfil junto al dato** (`~/Library/Application Support/abaco-deep-core/harness` **o** `~/Library/Application Support/dsh-desktop/harness`): **sin nombrar el perfil, el dato no se usa**. Método: `ls ~/Library/Application\ Support/` y mirar el **nombre** antes de leer nada. Detalle y retractaciones: §2.1, §2.2, §7. |

---

## 7. Verificado, inferido y no verificado

**Verificado por lectura directa (con cita):** todo el §2 — **salvo el error de perfil retractado más abajo**, donde el §2.1 leyó el `settings.yaml` del perfil vecino (`dsh-desktop`) creyendo leer el de ABACO —, incluidos los tres sitios de registro (`build/dsh-desktop.patch.yml:111-126`, `patches/@deepseek-ai+dsh+0.1.2-rc.1.patch:22-23`, `package.json:285-287`), la regla de efectos de Cordis (`$DSH_NM/cordis/lib/index.js:1139-1165`), la ausencia de `isError` en el pruner, y el camino de invocación `pruner.pruneSession` desde el motor (`$DSH_NM/dsh-compaction-basic/lib/index.js:868-885`).

**Correcciones a la auditoría recibida — con RETRACTACIÓN (2026-09-10):** de las cuatro, **(1) y (3) eran falsas y quedan retractadas**: las dos leyeron el perfil **vecino** (`…/dsh-desktop/harness`, la app DeepSeek Desktop) y se lo atribuyeron al de ABACO. **(1)** El `settings.yaml` del perfil de **ABACO** (`…/abaco-deep-core/harness/settings.yaml`) **hoy sí** contiene `agent-presets`, pero dice `default: abaco` (`:1-2`), y **no** `default: cordis` — cuyo `:11-12` es la cita del perfil **ajeno**. El hallazgo de la auditoría —que en ABACO la clave **faltaba**, y que por esa **ausencia** el default efectivo era el de la **composición**, `standard`— **era correcto** y es la causa raíz del Bug 2; hoy la clave está porque **su arreglo la escribió** (`7936852`). **(3)** El `$DSH_HOME` real de ABACO es `…/abaco-deep-core/harness`, **no** `…/dsh-desktop/harness`; la parte «no `~/.dsh`», en cambio, sí era cierta (`~/.dsh` **no existe**). **(2)** y **(4)** se mantienen en su núcleo, con salvedad de estado: en el perfil de ABACO **hoy existen** `abaco-memory/` y `.agent-presets/` —con `.abaco-context.json` **y** `.abaco-context-default.json`—, así que «no existe» y «ambos ausentes» ya no describen el disco actual. Ver §2.1, §2.2 y **R13**.
...
- El par de tokens se calcula en `resolveCompactSpec` (`$DSH_NM/dsh-compaction-basic/lib/index.js:111-112`): `thresholdTokens = floor(contextWindow * thresholdRatio)`; y para la retención **el orden de precedencia es el inverso del que se documentó antes**: la línea `:112` es `policy.retainTokens === undefined ? floor(contextWindow * policy.retainRatio) : policy.retainTokens`. Es decir: **si `policy.retainTokens` ESTÁ definido, gana ese valor absoluto**; el `floor(contextWindow × retainRatio)` se usa **solo** cuando `retainTokens` **no** está definido. El absoluto nunca es el respaldo: es la primera opción.
- **La proporción es válida con esta policy**: la propia línea `:113` exige `retainTokens < thresholdTokens`. Como la fila del lock fija **solo `retainRatio`** (ningún `retainTokens` absoluto), se aplica la rama de respaldo y `floor(contextWindow × 0.12) < floor(contextWindow × 0.90)` para todo window. ✅ **Condición de validez (consecuencia del precedente corregido):** en cuanto un `modelPolicies[i]` fije un `retainTokens` **absoluto**, ese valor **sustituye** al ratio y el `throw` de `:113` deja de ser inalcanzable — es el caso que describen §8.7 y §8.8. La seguridad de la policy depende de que **no haya absolutos**, no de que el ratio «pise» al absoluto.
...
| Hecho | Cita |
|---|---|
| `DEFAULT_CONTEXT_WINDOW = 1e6` | `$DSH_NM/dsh-llm-deepseek/lib/index.js:1378` |
...
| Modelo enrutado por defecto = `deepseek-official/deepseek-v4-flash` | La fila de composición `agent-default-model` en `$DSH_NM/dsh-base/cordis.patch.yml:75-79` (`provider: deepseek-official`, `model: deepseek-v4-flash`). **Ojo:** la sección `agent-default-model` de `$DSH_HOME/settings.yaml` **no existe** en el perfil ABACO — ese `settings.yaml` solo contiene `agent-presets` y `ui-onboarding` (3 líneas útiles, verificado el 2026-09-10), así que rige el valor de la composición; el settings solo lo superpone si el usuario guarda una selección propia |
