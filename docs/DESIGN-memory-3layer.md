...
- El fichero de composición es `agent.cordis.yml` (`dsh-agent-presets/lib/index.js:182`); el roster los
  ...
  **`$DSH_HOME/.agent-presets`** (`USER_PRESET_DIR` `:195`, defaults `includeShippedRoot/includeUserRoot`
  ...
  `default: standard`).
- ⚠️ **RETRACTADO (2026-09-10).** Aquí decía: *«**En la instalación observada el preset activo es `cordis`**, no `standard` (`<DSH_HOME>/settings.yaml` → `agent-presets: { default: cordis }`).»* **Era falso y estaba mal atribuido.** Ese `agent-presets: default: cordis` está en `~/Library/Application Support/`**`dsh-desktop`**`/harness/settings.yaml:11-12` — el perfil de la app **DeepSeek Desktop**, que **no** es ABACO — y se citó como si fuera `<DSH_HOME>` sin nombrar el perfil. En el perfil **de ABACO** (`…/abaco-deep-core/harness/settings.yaml`) la clave `agent-presets` **faltaba**, así que el default efectivo era el de la **composición**, `standard` (`dsh-web-app/cordis.patch.yml:440-444`); ésa es la causa raíz del Bug 2. **Hoy** ese `settings.yaml` dice `agent-presets: default: abaco` (`:1-2`), y lo dice **porque el arreglo del Bug 2 la escribió** (`7936852`), no porque hubiera estado siempre. **Regla: sin nombrar el perfil, el dato no se usa** — ver R13 en `docs/SPEC-CONTEXT-3-LAYERS.md` §6 y `docs/HANDOFF-TO-COLLABORATOR.md` §4.9.
