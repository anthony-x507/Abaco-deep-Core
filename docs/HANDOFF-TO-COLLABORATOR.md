...
> **Convención de rutas:** `$DSH_NM` = `/Applications/ABACO DEEP HARNES.app/Contents/Resources/app/node_modules/@deepseek-ai` — la copia del motor que **ABACO ejecuta**, y por tanto la **AUTORITATIVA** para toda cita `archivo:línea` de este documento. **No** es `/Applications/DSH Desktop.app/Contents/Resources/app/node_modules/@deepseek-ai`, que es el motor de la app **DeepSeek Desktop** — **otro producto**, sin autoridad sobre ABACO — y a la que esta abreviatura apuntó **por accidente**. Tercera copia, la **entrada de build** del repo: `desktop/src/dsh-desktop/node_modules/@deepseek-ai`. Hoy las tres son byte-idénticas y por eso ninguna cita está mal, pero la cita nombra **siempre** la autoritativa. Trampa y procedimiento si algún día divergen: `docs/HANDOFF-FASE5.md` §7.12.
>
...
>
> **Nota de verificabilidad (actualizada el 2026-09-10).** Cuando se escribió esta nota, el perfil de ABACO estaba borrado y su `settings.yaml` **no se podía re-comprobar en disco**. **Hoy sí se puede**, y por eso la nota se retira: el perfil existe y su `settings.yaml:1-2` dice `agent-presets: default: abaco`. Lo que sigue siendo cierto es *por qué* la confusión se repite: el archivo que contiene justo el dato que el de ABACO no tenía (`default: cordis`, `:11-12`) es el del perfil vecino, `dsh-desktop`.
>
> ⚠️ **La spec decía lo contrario — ya está corregida (2026-09-10).** `docs/SPEC-CONTEXT-3-LAYERS.md:66` afirmaba que *"`settings.yaml` **SÍ** tiene la clave `agent-presets`: `$DSH_HOME/settings.yaml:11-12` → `default: cordis`"*, presentándolo como corrección a una auditoría anterior; y `:251` añadía que *"`$DSH_HOME` real es `…/dsh-desktop/harness`"*. Las dos lecturas **no pueden ser ciertas a la vez**, y la que correspondía al perfil de ABACO era la de este handoff. Quedó registrado como **D-5** en `docs/COLLABORATOR-CRITERIA.md` §13.3 y **está RESUELTA a favor de este documento**: la spec había leído el perfil **vecino**. Sus tres afirmaciones —`SPEC:66`, `SPEC:67` y la repetición que entonces vivía en `SPEC:251`— quedaron **retractadas** en el árbol de trabajo el 2026-09-10, conservando el texto falso **citado dentro** de cada retractación. Hoy viven en `SPEC:67`, `SPEC:68` y `SPEC:261` (la propia corrección desplazó las líneas), y la spec añadió **R13** a su tabla de riesgos. **Falta el commit**, que debe ser propio. Prueba en disco del default anterior: `…/abaco-deep-core/harness/.agent-presets/.abaco-context-default.json` → `{"applied": true, "presetId": "abaco", "replaced": "standard", "at": "2026-09-10T21:34:14.122Z"}`.

### 4.6 La capa 2 está vacía

`$DSH_HOME/abaco-memory/` — en el perfil **de ABACO**, `…/abaco-deep-core/harness` (R13) — **no existía**. La capacidad está construida y probada; el uso es **cero**. *(2026-09-10: el directorio **sí existe** hoy —lo crea la app al arrancar—, pero sigue **vacío**: un `vault/` sin un solo fichero. **El uso sigue siendo cero**.)* No hay evidencia de que la inyección de system prompt funcione de extremo a extremo, porque nunca hubo nada que inyectar.
...
- **R13 — la trampa de los dos perfiles (documentada; el caso de la spec quedó resuelto el 2026-09-10).** Los dos perfiles **se llaman casi igual**, viven en el **mismo directorio padre** (`~/Library/Application Support/`) y **ambos contienen una carpeta `harness/`**. Tres auditorías distintas los han confundido, y este documento fue una de ellas: leyó `agent-presets: default: cordis` del perfil de **DeepSeek Desktop** y se lo atribuyó al de ABACO (§4.5). Forma exacta de distinguirlos — listar el directorio padre y mirar el **nombre**:
  ```bash
  ls -la "$HOME/Library/Application Support" | grep -i 'abaco\|dsh'
  ```
  - `abaco-deep-core/` → perfil **de ABACO**, y su harness es `abaco-deep-core/harness`. Es **el nuestro**. *(**Corregido el 2026-09-10:** aquí decía «Hoy **no existe**: se borró en una limpieza posterior». **Sí existe** — volvió a crearse al arrancar la app —, y su `settings.yaml:1-2` dice hoy `agent-presets: default: abaco`.)* Fijado en código en `desktop/src/dsh-desktop/src/main/index.ts:533`, con la separación explicada en `:523-533`.
  - `dsh-desktop/` → perfil de la **app DeepSeek Desktop** (harness en `dsh-desktop/harness`). **No es ABACO.** Contiene `agent-presets: default: cordis` (`settings.yaml:11-12`) — **y ésa es, palabra por palabra, la cita que se le atribuyó por error al perfil de ABACO.**
