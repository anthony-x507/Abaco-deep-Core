...
function render(): void {
  ...
  card.setAttribute('aria-label', locale === 'zh' ? 'ABACO Deep Core 更新' : 'ABACO Deep Core update')
...
function renderAbout(): void {
  ...
  card.setAttribute('aria-label', zh ? '关于 ABACO Deep Core' : 'About ABACO Deep Core')

  // Header row with Title and Close '×'
  const header = element('div', 'about-header')
  const title = element('h2', 'about-title')
  title.textContent = zh ? '关于 ABACO Deep Core' : 'About ABACO Deep Core'
  ...
  line1.textContent = `${zh ? 'ABACO Deep Core 版本： ' : 'ABACO Deep Core version: '}${info.desktopVersion}`
  ...
  hint.textContent = zh ? 'Harness 随 ABACO Deep Core 更新。' : 'Harness is updated with ABACO Deep Core.'
