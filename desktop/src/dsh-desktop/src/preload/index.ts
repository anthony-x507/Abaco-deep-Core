...
function render(): void {
  ...
  card.setAttribute('aria-label', locale === 'zh' ? 'ABACO DEEP HARNES 更新' : 'ABACO DEEP HARNES update')
...
function renderAbout(): void {
  ...
  card.setAttribute('aria-label', zh ? '关于 ABACO DEEP HARNES' : 'About ABACO DEEP HARNES')

  // Header row with Title and Close '×'
  const header = element('div', 'about-header')
  const title = element('h2', 'about-title')
  title.textContent = zh ? '关于 ABACO DEEP HARNES' : 'About ABACO DEEP HARNES'
  ...
  line1.textContent = `${zh ? 'ABACO DEEP HARNES 版本： ' : 'ABACO DEEP HARNES version: '}${info.desktopVersion}`
  ...
  hint.textContent = zh ? 'Harness 随 ABACO DEEP HARNES 更新。' : 'Harness is updated with ABACO DEEP HARNES.'
