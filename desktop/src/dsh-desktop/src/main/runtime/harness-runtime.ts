...
export interface HarnessRuntimeOptions {
  ...
  /**
   * Values merged into the Harness child's environment at every spawn.
   * Read at launch time so a safe-mode or plugin-reset relaunch picks up
   * the current control-plane port and token instead of a stale pair.
   */
...
export class HarnessRuntime {
  ...
  async start(launchDirectory: string, profile = 'web'): Promise<void> {
    ...
    this.setState('starting', 'Starting ABACO DEEP HARNES…')
