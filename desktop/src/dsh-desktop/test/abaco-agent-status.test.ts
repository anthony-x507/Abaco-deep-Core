...
describe('abaco-agent-status composer watermark', () => {
  it('hooks SessionSnapshot.running and sizes the mark at 70% with a 2s pulse', async () => {
    ...
    expect(source).toContain('width: 70%')
    ...
    expect(source).toContain('findComposerHost')
    ...
    expect(styleNodes[0]?.textContent).toContain('width: 70%')
    expect(styleNodes[0]?.textContent).toContain('2000ms')
  })
})
