...
const sampleModuleTableError =
  'failed to import loader entry 8009188ec (abaco-analytics): client-modules: require("./lib/summary.js") missed the module table (build-time externals drift or dynamic dependency)'
...
describe('plugin load error detection and extraction', () => {
  it('detects bundle script and loader entry failure errors', () => {
    ...
    expect(isPluginLoadError(new Error(sampleModuleTableError))).toBe(true)
  ...
  it('extracts plugin name from error messages', () => {
    ...
    expect(extractPluginName(sampleModuleTableError)).toBe('abaco-analytics')
