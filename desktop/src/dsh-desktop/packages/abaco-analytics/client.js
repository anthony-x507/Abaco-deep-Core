window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Self-contained summary helpers (inlined from lib/summary.js).
    // The web module table only seeds platform ids (react, @deepseek-ai/*),
    // so no relative lib import may appear in this bundle. A relative
    // import here is the exact 0.4.22+ boot: "./lib/summary.js missed the
    // module table" → failed to import loader entry → Startup recovery.
    // Keep the copies in lib/summary.js for Node tests; CI fails if a
    // relative require returns.
    ...
    function readUsage(usage) {
      ...
      const cached = firstNumber(usage, [
        'cacheReadTokens',
        'cached_tokens',
    ...
    function warnApply(message) {
      try {
        console.warn(`abaco-analytics: ${message}`)
      } catch {
        /* a console that throws must not fail boot */
      }
    }

    function startAnalytics(ctx) {
      injectStyle()
      watchComposerDock()
    ...
    // Soft-apply the renderer half. A slot/register throw after a successful
    // import must degrade this page, not take the plugin tree to Startup
    // recovery. Disabled plugins stay disabled — this catch does not rewrite
    // the patch layer.
    function apply(ctx) {
      try {
        startAnalytics(ctx)
        ...
        warnApply(
          `failed to start; Settings → Analytics is off: ${error && error.message ? error.message : error}`,
        )
      }
    }
