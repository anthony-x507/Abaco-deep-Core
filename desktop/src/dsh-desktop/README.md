<h1 align="center">
  <img src="build/logo-light.png" width="64" alt="ABACO DEEP HARNES logo" valign="middle" />
  ABACO DEEP HARNES
</h1>
...
![ABACO DEEP HARNES overview with portable presets, model providers, phone control, and editable PPT generation](docs/images/dsh-desktop-hero-v021.png)
...
ABACO DEEP HARNES packages the local DeepSeek Harness experience as an installed desktop application. It starts Harness automatically, keeps profiles, plugins, workspaces, model settings, and sessions outside the application directory, and opens the full Harness interface as soon as the local runtime is ready.

> [!IMPORTANT]
> ABACO DEEP HARNES is a personal fork of [DataElement/dsh-desktop](https://github.com/DataElement/dsh-desktop) (MIT). It is an early preview built on the rapidly evolving `@deepseek-ai/dsh@0.1.2-rc.1`. macOS releases are code-signed and notarized by Apple. Windows x64 installers are code-signed; Windows security warnings may still decrease gradually as the publisher builds download and installation reputation.
...
ABACO DEEP HARNES ships its own Cordis plugins on top of upstream's set:

| Plugin | Purpose | Status |
|---|---|---|
| `abaco-theme` | Design tokens (color, type, spacing, motion) shared by all plugins | ✅ |
| `abaco-brand` | Brand metadata + sidebar tweaks | ✅ |
| `abaco-device-identity` | Stable per-Mac UUID in macOS Keychain | ✅ |
| `abaco-voice` | TTS / STT with multi-provider menu (Web Speech, OpenAI, Edge, ElevenLabs, Deepgram) | ✅ |
| `abaco-documents` | PDF / DOCX / TXT / MD / CSV upload with client-side text extraction | ✅ |
| `abaco-cloud-sync` | Multi-Mac sync (backend pending Fase 3) | 🟡 skeleton |
| `abaco-onboarding` | First-run wizard | 🟡 skeleton |
| `abaco-experimental` | Opt-in feature flags | 🟡 skeleton |

Planned for Fase 2: `abaco-brands`, `abaco-tools`, `abaco-customer-db`,
`abaco-secure-storage`, `abaco-key-calculator` — domain plugins for the
locksmith / automotive workflow.

## Privacy

- TTS / STT providers are selectable from the settings menu. Web Speech (browser) keeps audio on device.
- Cloud providers (OpenAI, Edge, ElevenLabs, Deepgram) only see audio when you actively use them — a disclosure dialog appears the first time.
- API keys live in the macOS Keychain via `abaco-device-identity`'s secure store, never in plain `settings.yaml`.
- Document text extraction is 100% client-side; binary file content never leaves the device.

## Attribution

This project is a fork of [DataElement/dsh-desktop](https://github.com/DataElement/dsh-desktop), © DataElement, used under the [MIT License](LICENSE). See [NOTICE](NOTICE) for full attribution.

## What ABACO DEEP HARNES adds

DeepSeek Harness already provides the Agent runtime and Web UI. ABACO DEEP HARNES adds the native host capabilities needed for a practical desktop product:
...
If a third-party plugin interferes with startup or rendering, ABACO DEEP HARNES can identify the implicated plugin from runtime and frontend evidence and open a guided recovery surface.
...
If the normal interface cannot be reached, start ABACO DEEP HARNES with `--safe-mode`. On macOS:

```sh
open -a "ABACO DEEP HARNES" --args --safe-mode
```
...
ABACO DEEP HARNES is open source under the [MIT License](LICENSE).

DeepSeek Harness and its dependencies remain subject to their respective upstream licenses and trademark policies. ABACO DEEP HARNES is a personal fork maintained by Anthony Sanchez.
